...
<div class="app">
  ...
  <div id="gps-banner">
    ...
    <div class="gps-info">
      <div class="gps-label">Ubicación actual</div>
      <div class="gps-value" id="gps-text">Sin ubicación — requerida para registrar</div>
    </div>
  ...
  <div class="tabs">
    <button class="tab-btn active" id="tab-btn-visita" onclick="switchTab('visita')">📋 Registrar Visita</button>
    <button class="tab-btn" id="tab-btn-cliente" onclick="switchTab('cliente')">➕ Nuevo Cliente</button>
  </div>
    ...
    <div class="card">
      ...
      <div class="field">
        ...
        <select id="f-cl" disabled>
          <option value="">— Selecciona ejecutivo primero —</option>
        </select>
      </div>
      <div class="field">
        <label>Resultado <span class="req">*</span></label>
        <select id="f-res" disabled>
          <option value="">— Cargando resultados... —</option>
        </select>
      </div>
    </div>
    <div class="card">
      <div class="card-title">Evidencia Fotográfica <span class="req">*</span></div>
      <div class="photo-zone" id="photo-zone" onclick="openCamera()">
        ...
        <div class="ph"><div class="icon">📷</div><p>Tomar foto</p><span>Obligatoria — solo cámara</span></div>
      </div>
    </div>
    <button class="sbtn" id="btn-registrar" onclick="submitVisit()">✓ &nbsp; Registrar Visita</button>
  </div>
  ...
  <div class="tab-panel" id="tab-cliente">
    <div class="card">
      ...
      <div class="field">
        ...
        <select id="nc-ej"><option value="">— Selecciona ejecutivo —</option></select>
      </div>
      <div class="field">
        <label>Nombre del cliente <span class="req">*</span></label>
        <input type="text" id="nc-nom" placeholder="Ej: Farmacia San José" oninput="sanTxt(this)">
        <div class="field-hint">Solo letras, números y espacios</div>
      ...
      <div class="field-row">
        ...
        <div class="field">
          <label>Cédula / NIT <span class="req">*</span></label>
      ...
      <div class="field">
        ...
        <div class="field-hint">Solo letras, números, espacios y puntos</div>
        <div class="ferr" id="err-dir"></div>
      </div>
      <div class="field-row">
        <div class="field">
          <label>Ciudad <span class="req">*</span></label>
          <input type="text" id="nc-ciu" placeholder="Bogotá" oninput="sanTxt(this)">
    ...
    <button class="sbtn" id="btn-cliente" onclick="guardarCliente()">➕ &nbsp; Guardar Cliente</button>
  </div>
  ...
  <div class="logcard">
    ...
    <div id="vlist"><div class="emptylog">Aún no hay visitas registradas</div></div>
    <button class="expbtn" onclick="exportCSV()">⬇ Exportar CSV</button>
  </div>
</div>
...
<script>
const URL = "https://script.google.com/macros/s/AKfycbzKKEY60FtX9NkpsMC4Ve9k_IA51NiUeWWW1YQ8RHNXG11SIH2Ba3pk5z-26cMm6RBg/exec";
...
function cargarCatalogo() {
  ...
  st.style.display = "block"; st.textContent = "⏳ Cargando...";
  jsonp(URL + "?action=catalogo", function(err, d) {
    if (err || d.error) { st.textContent = "⚠️ Error cargando ejecutivos"; return; }
    ...
    st.textContent = "✓ " + Object.keys(d).length + " ejecutivos";
...
function cargarResultados() {
  jsonp(URL + "?action=resultados", function(err, d) {
...
// ── Ejecutivo cambia → cargar clientes
function onEjChange() {
...
function captureGPS() {
  ...
  navigator.geolocation.getCurrentPosition(
    function(p) {
      ...
      btn.textContent = "✓ Listo"; btn.style.background = "#008a70";
...
function handlePhoto(e) {
  ...
  if (!f) return;
  if (f.size > 5*1024*1024) { toast("Foto muy grande (máx 5MB)","err"); return; }
  var r = new FileReader();
  r.onload = function(ev) {
    foto = ev.target.result;
    var z = document.getElementById("photo-zone");
    z.className = "photo-zone has-photo";
    z.innerHTML = "<input type='file' accept='image/*' capture='environment' id='photo-input' onchange='handlePhoto(event)' style='display:none'><img src='" + foto + "' alt='' onclick='openCamera()' style='cursor:pointer;width:100%;height:170px;object-fit:cover;border-radius:12px'><div class='photo-overlay'>📷 Cambiar</div>";
  };
  r.readAsDataURL(f);
}

// ── Sanitización
function sanTxt(i){ i.value=i.value.replace(/[^a-zA-Z0-9áéíóúÁÉÍÓÚñÑüÜ\s]/g,""); }
function sanDir(i){ i.value=i.value.replace(/[^a-zA-Z0-9áéíóúÁÉÍÓÚñÑ\s\.#]/g,""); }
...
function submitVisit() {
  ...
  if (!gps) { toast("GPS obligatorio — presiona Obtener GPS","err"); return; }
  ...
  jsonp(URL + "?" + p.toString(), function(err, d) {
    btn.disabled = false;
    btn.innerHTML = "✓ &nbsp; Registrar Visita";
    if (!err && d && d.success) {
      toast("✅ Visita registrada en Sheets");
      // Subir foto aparte
      log("VISITA OK: llamando subirFoto con id=" + id + " foto=" + (foto ? "SI" : "NO"));
    if (foto) subirFoto(id, foto);
    else log("FOTO: no hay foto para subir");
    } else {
      toast("⚠️ " + (d && d.error ? d.error : "Error de conexión"), "err");
    }
    // Reset solo GPS y foto — mantener ejecutivo
    resetParcial();
  });
}

// Reset solo GPS, foto y cliente — mantiene ejecutivo seleccionado
function resetParcial() {
  gps = null; foto = null;
  // Reset GPS UI
  document.getElementById("gps-text").textContent = "Sin ubicación — requerida para registrar";
  ...
  document.getElementById("photo-zone").innerHTML = "<input type='file' accept='image/*' capture='environment' id='photo-input' onchange='handlePhoto(event)' style='display:none'><div class='ph'><div class='icon'>📷</div><p>Tomar foto</p><span>Obligatoria — solo cámara</span></div>";
  // Reset cliente — recargar lista del ejecutivo actual
...
// ── Subir foto: comprimir y enviar base64 como texto plano
function subirFoto(visitId, b64) {
  log("FOTO: iniciando subida para visita " + visitId);
  log("FOTO: tamaño original b64 = " + Math.round(b64.length/1024) + "KB");
  try {
    var img = new Image();
    img.onload = function() {
...
function guardarCliente() {
  ...
  if (tel.length < 7) { setErr("err-tel","Mínimo 7 dígitos"); ok=false; }
  if (ced.length < 6) { setErr("err-ced","Mínimo 6 dígitos"); ok=false; }
  ...
  jsonp(URL + "?" + p.toString(), function(err, d) {
    btn.disabled=false; btn.innerHTML="➕ &nbsp; Guardar Cliente";
    if (err)          { toast("Error de conexión","err"); return; }
...
function renderVisitas() {
  ...
  if (!visitas.length) { list.innerHTML="<div class='emptylog'>Aún no hay visitas registradas</div>"; return; }
  list.innerHTML = visitas.map(function(v){
    var pc = pillMap[v.res] || "p3";
    return "<div class='vi'>" +
      (v.fotoData ? "<img class='vthumb' src='"+v.fotoData+"' alt=''>" : "<div class='vnothumb'>🏢</div>") +
      "<div class='vdet'>" +
      "<div class='vclient'>" + v.cl + "</div>" +
      "<div class='vmeta'><span>"+v.ej+"</span><span class='pill "+pc+"'>"+v.res+"</span></div>" +
      "<div class='vmeta' style='margin-top:3px'><span>🕐 "+v.fecha+"</span>" +
      "<a href='"+v.mapsUrl+"' target='_blank' style='color:var(--accent);text-decoration:none'>📍 Mapa</a></div>" +
      "</div></div>";
  }).join("");
}
